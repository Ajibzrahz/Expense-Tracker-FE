    ...
    (async () => {
      try {
        const [sum, ov, tr, tx, cats] = await Promise.all([
          ...
          api<{ success: boolean; data: OverviewRow[] }>(
            "/analytics/overview",
          ).catch(() => ({ data: [] })),
          api<{ success: boolean; data: TrendRow[] }>(
            "/analytics/trends",
          ).catch(() => ({ data: [] })),
      ...
      <div className="kpis">
        <div className="kpi">
          <div className="kpi-row">
            ...
            <span className="kpi-ic income">
              <TrendingUp size={18} />
            </span>
          </div>
          <div className="val income">{money(s.totalIncome)}</div>
        </div>
        <div className="kpi">
          <div className="kpi-row">
            <div className="lbl">Expenses</div>
            <span className="kpi-ic expense">
              <TrendingDown size={18} />
            </span>
          </div>
          <div className="val expense">{money(s.totalExpense)}</div>
        </div>
        <div className="kpi">
          <div className="kpi-row">
          ...
          <div className="val">{money(s.balance)}</div>
        </div>
        <div className="kpi">
          <div className="kpi-row">
          ...
          <div className="val">{s.savingsRate}%</div>
        </div>
      </div>
      ...
      <div className="charts">
        <div className="panel">
          ...
          {activeOverview.length === 0 ? (
            <Empty msg="No activity yet this year." />
          ) : (
            <OverviewChart rows={activeOverview} />
          )}
        </div>
        <div className="panel">
          <div className="panel-h">
          ...
          {trend.every((t) => t.total === 0) ? (
            <Empty msg="No expenses this week." />
          ) : (
          ...
          return (
            <g key={f}>
              <line
                ...
                className="grid"
              />
              <text x={pad.l - 8} y={gy + 4} className="axis" textAnchor="end">
                {Math.round(max * f)}
              </text>
            </g>
          );
          ...
          return (
            <g key={r.month}>
              ...
              <rect
                ...
                y={y(r.expense)}
                width={bw}
                height={pad.t + ih - y(r.expense)}
                className="b-expense"
                rx={3}
              />
              <text x={cx} y={H - 9} className="axis" textAnchor="middle">
                {r.month}
              </text>
            </g>
          );
...
function TrendChart({ rows }: { rows: TrendRow[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="chart">
      ...
      {rows.map((r, i) => (
        <g key={r.date}>
          <circle cx={x(i)} cy={y(r.total)} r={3.5} className="tdot" />
          <text x={x(i)} y={H - 9} className="axis" textAnchor="middle">
            {dl(r.date)}
          </text>
        </g>
      ))}
